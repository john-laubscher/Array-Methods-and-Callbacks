...
function getWinnersByYear(arr, getYearscb, getWinnerscb) {
  const getYearsArr = getYearscb(arr);
  const getCountryArr = getWinnerscb(arr);
  const newArray = [];
  getYearsArr.forEach(function (item, index) {
    return newArray.push(
      `In ${item}, ${getCountryArr[index]} won the world cup!`
    );
  });
  return newArray;
}
//getYearsArr.map(function(item, index){
...
function getAverageGoals(arr) {
  //     let homeGoals = getFinals(arr).map(function(item){
  //         return item['Home Team Goals']
  //     });
  //     let awayGoals = getFinals(arr).map(function(item){
  //         return item['Away Team Goals']
  //     });
  //     (homeGoals).reduce(function(accum, currentValue){
  //    let avgGoals = Math.round(100*(homeGoals / homeGoals.length)) / 100;
  //     });
  //    (awayGoals).reduce(function(accum, currentValue){
  //     let avgGoals = Math.round(100*(awayGoals / awayGoals.length)) / 100;
  //    });
  //     console.log(getAverageGoals(getFinals(fifaData)))
  let total = arr.reduce(function (accum, currentValue) {
    return (
      accum + currentValue["Home Team Goals"] + currentValue["Away Team Goals"]
    );
    //console.log(total);
  },0);
